...
document.getElementById('calculateBtn').addEventListener('click', async () => {
    ...
    try {
        ...
        depMarker = L.marker([depData.lat, depData.lon]).addTo(map).bindPopup(`${dep} (${depData.name})`).openPopup();
        arrMarker = L.marker([arrData.lat, arrData.lon]).addTo(map).bindPopup(`${arr} (${arrData.name})`);

        line = L.polyline([[depData.lat, depData.lon], [arrData.lat, arrData.lon]], { color: 'red' }).addTo(map);

        // Fit map bounds
        map.fitBounds([[depData.lat, depData.lon], [arrData.lat, arrData.lon]], { padding: [50, 50] });
